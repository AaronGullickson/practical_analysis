...
Before we talk about actually reading in data, we need to have a conversation about something else. I am assuming in this chapter that you are reading in data from a file that you have locally on your system. This isn't always tru as you can read data from online databases and other online sources as well, like Google Sheets. However, in most cases, you will have downloaded data or an extract of a larger dataset to your local computer. 
...
We want to rename these variables to something more approporiate. There are a couple of different techniques that we can use to rename the variables. The first approach is to apply the `rename` function to the dataset after we load the dataset into *R*. The general syntax of `rename` is:
...
Lets move from this toy example to another real world case. I have downloaded the 2022 American Community Survey data from the [IPUMS](https://usa.ipums.org/usa/) website. Rather than download the very large full data, I downloaded a small 500,000 person sample of the full data for demonstration purposes. You can download the data [here](https://github.com/AaronGullickson/practical_analysis/blob/master/data/usa_00129.dat.gz) but note that it is currently compressed as a "gzip" file with a "*.gz" suffix. One nice feature of the `read_` functions in `readr` is that they can all read directly from compressed data files. This feature can be extremely useful when you have a large dataset.

If I were to uncompress my file and look at it, here is what the first five lines would look like:

```md

```
...
```{r}
acs <- read_fwf("data/usa_00129.dat.gz",
                col_positions = fwf_cols(perwt   = c(74,83),
                                         sex     = c(84,84),
                                         age     = c(85,87),
                                         marst   = c(88,88),
                                         race    = c(89,89),
...
```{r}
acs <- read_fwf("data/usa_00129.dat.gz",
                col_positions = fwf_cols(cluster = c(42,54),
                                         perwt   = c(74,83),
                                         sex     = c(84,84),
                                         age     = c(85,87),
                                         marst   = c(88,88),
...
# A tibble: 262,657 × 5
   ...
   <int> <int> <chr>      <chr>         <chr>
 1     1     1 an integer 2022000000071 ""   
 2     1     1 an integer 2022000000131 ""   
 3     1     1 an integer 2022000000201 ""   
 4     1     1 an integer 2022000000271 ""   
 5     1     1 an integer 2022000000341 ""   
 6     1     1 an integer 2022000000401 ""   
 7     1     1 an integer 2022000000471 ""   
 8     2     1 an integer 2022000000541 ""   
 9     2     1 an integer 2022000000611 ""   
10     2     1 an integer 2022000000671 ""   
# ℹ 262,647 more rows
...
Most of these formats are data saved in another statistical software program, such as the *.dta files that Stata saves. Additionally, you will often come across data in a spreadsheet format saved in Microsoft Excel, or ocassionally OpenOffice.
...
If you can read data, it also makes sense that you can write data. All of the functions I have described here have a corrollary `write_` function. So, for example, if you want to output data in CSV format, you can use the `readr::write_csv()` function.
